...
that maps a global ``ncells_idx`` to (image group, row within that image). Because
``ncells_idx`` is contiguous 0..N-1 (guaranteed by the migration), it also equals
the row position in every ``cell_index`` column — so a cell is addressed by a
single integer. We therefore work entirely in terms of **lists of global
indices**: shuffle/split them, then read exactly one chunk per cell. No per-split
...
Only needs numpy, torch, zarr and lightning (CSV metadata is read with the
stdlib). Point ``data_path`` at the zarr store; nothing is hard-coded.
...
# Routing columns read from cell_index (kept as in-memory numpy arrays).
_ROUTING_COLS = ("ncells_idx", "local_cell_index", "local_label_id",
                 "replicate", "condition", "image_name")
# Metadata fields always returned per sample (straight from cell_index).
...
# ════════════════════════════════════════════════════════════════════════════
# cell_index / metadata loading (no pandas)
# ════════════════════════════════════════════════════════════════════════════

def load_cell_index(data_path) -> dict[str, np.ndarray]:
    """Read the cell_index routing columns into a dict of numpy arrays.

    Arrays are ordered by (and therefore indexable by) global ``ncells_idx``.
    """
    root = zarr.open_group(str(data_path), mode="r")
    ci = root["cell_index"]
    routing = {}
    for col in _ROUTING_COLS:
        arr = ci[col][:]
        routing[col] = arr.astype(str) if arr.dtype == object else arr
    return routing


def _maybe_num(s: str):
    """Cast a CSV string to int/float when possible, else leave it as a string."""
    try:
        f = float(s)
...
def load_image_metadata(csv_path, fields: Sequence[str]) -> dict[tuple, dict]:
    """Build {(replicate, image_name): {field: value}} from the microscopy CSV.

    Joins on ``sample`` -> replicate and ``stem(filename)`` -> image_name, matching
    the cell_index. Numeric fields are cast; missing/empty values are skipped.
    """
    lookup: dict[tuple, dict] = {}
    with open(csv_path, newline="") as f:
        for row in csv.DictReader(f):
            key = (row["sample"], Path(row["filename"]).stem)
            lookup[key] = {fld: _maybe_num(row[fld]) for fld in fields
                           if row.get(fld, "") != ""}
    return lookup


def _py(v):
    """Convert a numpy scalar to a native Python type (for clean metadata dicts)."""
...
class CellPatchDataset(Dataset):
    """One cell per item, addressed by a list of global ``ncells_idx``.

    Routing (which image group + row) and per-cell metadata are read from the
    in-memory ``routing`` arrays — indexed directly by the global index — so no
    per-split table is built. Normalisation lives in ``transform`` (it needs the
    cell mask), so ``norm_mask_array`` is always loaded into the private
    ``_normmask`` key, passed through the pipeline, then dropped before return.
    ...
    indices : sequence of int
        Global ``ncells_idx`` values this dataset serves.
    routing : dict[str, np.ndarray]
        Full cell_index columns from :func:`load_cell_index`.
    ...
    transform : callable or None
        Sample-dict transform (normalisation + augmentation). If None the image
        is returned RAW (unnormalised).
    norm_mask_array : str
        Mask array used for per-cell normalisation (default "cCellmask").
    metadata_fields : sequence of str
        Routing columns to return under sample["metadata"].
    image_metadata : dict or None
        {(replicate, image_name): {field: value}} from :func:`load_image_metadata`,
        merged into sample["metadata"] when present.
    """

    def __init__(self, data_path, indices: Sequence[int], routing: dict[str, np.ndarray],
                 patch_type: str = "cPatches", channels: Sequence[int] | None = (0, 1),
                 masks: Sequence[str] = (), transform: Callable | None = None,
        ...
        self.metadata_fields = tuple(f for f in metadata_fields if f in routing)
    ...
    def __getitem__(self, i: int) -> dict:
        self._ensure_open()
        g = int(self.indices[i])                       # global ncells_idx == row position
        rep = str(self.routing["replicate"][g])
        cond = str(self.routing["condition"][g])
        img = str(self.routing["image_name"][g])
        loc = int(self.routing["local_cell_index"][g])
        ...
        meta = {f: _py(self.routing[f][g]) for f in self.metadata_fields}
        if self.image_metadata is not None:
            meta.update(self.image_metadata.get((rep, img), {}))
...
# ════════════════════════════════════════════════════════════════════════════
# Splitting (operates on routing arrays — returns lists of global indices)
# ════════════════════════════════════════════════════════════════════════════

def build_splits(routing: dict[str, np.ndarray], ratios=(0.7, 0.15, 0.15),
    ...
    r_tr, r_va, _ = ratios
    ncells = np.asarray(routing["ncells_idx"], dtype=np.int64)
    ...
    rep = routing["replicate"]
    img = routing["image_name"]
    strat = routing[stratify_by] if stratify_by else None

    # Map each image -> its cell indices, and image -> stratum (insertion order).
    cells_by_image: dict[tuple, list] = defaultdict(list)
    image_stratum: dict[tuple, str] = {}
    for i in range(len(ncells)):
        key = (rep[i], img[i])
        cells_by_image[key].append(int(ncells[i]))
        if key not in image_stratum:
            image_stratum[key] = str(strat[i]) if strat is not None else "_all"

    images_by_stratum: dict[str, list] = defaultdict(list)
    for key, s in image_stratum.items():
        images_by_stratum[s].append(key)

    out = {"train": [], "val": [], "test": []}
    for keys in images_by_stratum.values():
        keys = [keys[j] for j in rng.permutation(len(keys))]
        n = len(keys)
        n_tr, n_va = int(round(n * r_tr)), int(round(n * r_va))
        for bucket, chosen in (("train", keys[:n_tr]),
                               ("val", keys[n_tr:n_tr + n_va]),
                               ("test", keys[n_tr + n_va:])):
            for key in chosen:
                out[bucket].extend(cells_by_image[key])
...
class MultinucDataModule(LightningDataModule):
    ...
    augment : bool
        Apply training augmentations. Val/test never augment. Ignored if a custom
        ``train_transform`` is given.
    ...
    image_metadata_csv : str or None
        Optional microscopy CSV; joined per cell to attach bias fields to
        sample["metadata"].
    metadata_fields : sequence of str
    ...
    def __init__(
        ...
        self.routing: dict[str, np.ndarray] | None = None
        self.image_metadata: dict | None = None
    ...
    def _make_dataset(self, split_idx: np.ndarray, transform) -> CellPatchDataset:
        return CellPatchDataset(
            self.data_path, split_idx, self.routing, patch_type=self.patch_type,
            channels=self.channels, masks=self.masks, transform=transform,
            norm_mask_array=self.norm_mask_array, metadata_fields=_CORE_META,
    ...
    def setup(self, stage: str | None = None):
        if self.routing is None:
            self.routing = load_cell_index(self.data_path)
            if self.image_metadata_csv:
                self.image_metadata = load_image_metadata(self.image_metadata_csv, self.metadata_fields)
        if self.splits is None:
            self.splits = build_splits(
                self.routing, ratios=self.split_ratios, split_by=self.split_by,
...
def compute_normalization_stats(data_path, indices, patch_type="cPatches",
    ...
    """
    routing = load_cell_index(data_path)
    root = zarr.open_group(str(data_path), mode="r")
    ...
    acc = [[] for _ in ch]
    for g in idx:
        rep, cond = routing["replicate"][g], routing["condition"][g]
        img, loc = routing["image_name"][g], int(routing["local_cell_index"][g])
        arr = root[f"patches/{rep}/{cond}/{img}"][patch_type][loc]    # (H, W, C)
